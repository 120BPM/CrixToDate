setwd("C:/Users/sfb649/Documents/R2/CrixToDate-master")
...
mean_sd_list = list()
for (crypto in colnames(PRICE_max_xts)) {
  mean_sd_list[[crypto]] = mean_sd_window_comp(RET_max_xts[,crypto], 
                                               window_length)
}


names(color) = colnames(PRICE_max_xts)


for (j in 1:length(colnames(PRICE_max_xts))) {
  pdfname = paste("Mean moving window 180", gsub(" ", "", 
                                                 colnames(PRICE_max_xts)[j], fixed = TRUE), ".pdf")
  pdf(file = pdfname)
  plot(zoo(mean_sd_list[[j]][, 1]), type = "l", col = color[j], 
       lwd = 3, main = paste("Means in rolling windows: ", 
                             gsub(" ", "", colnames(PRICE_max_xts)[j], fixed = TRUE)), 
       ylab = "Mean log returns", prob=TRUE, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  dev.off()
  pdfname2 = paste("Volatility moving window 180", gsub(" ", 
                                                        "", colnames(PRICE_max_xts)[j], fixed = TRUE), ".pdf")
  pdf(file = pdfname2)
  plot(zoo(mean_sd_list[[j]][, 2]), type = "l", col = color[j], 
       lwd = 3, main = paste("Standart deviation in rolling windows: ", 
                             gsub(" ", "", colnames(PRICE_max_xts)[j], fixed = TRUE)), 
       ylab = "Standart deviation log returns",  prob=TRUE, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  dev.off()
  pdfname3 = paste("Skewness moving window 180", gsub(" ", 
                                                        "", colnames(PRICE_max_xts)[j], fixed = TRUE), ".pdf")
  pdf(file = pdfname3)
  plot(zoo(mean_sd_list[[j]][, 3]), type = "l", col = color[j], 
       lwd = 3, main = paste("Skewness in rolling windows: ", 
                             gsub(" ", "", colnames(PRICE_max_xts)[j], fixed = TRUE)), 
       ylab = "Skewness log returns",  prob=TRUE, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  dev.off()
  pdfname4 = paste("Kurtosis moving window 180", gsub(" ", 
                                                        "", colnames(PRICE_max_xts)[j], fixed = TRUE), ".pdf")
  pdf(file = pdfname4)
  plot(zoo(mean_sd_list[[j]][, 4]), type = "l", col = color[j], 
       lwd = 3, main = paste("Kurtosis deviation in rolling windows: ", 
                             gsub(" ", "", colnames(PRICE_max_xts)[j], fixed = TRUE)), 
       ylab = "Kurtosis log returns",  prob=TRUE, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  dev.off()
}
